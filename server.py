...
VERSION="75.8"
BUILD_NAME="ATLAS 75.8 REFINED COMMAND CENTER"
...
def atlas_ai_ask(question):
    ...
    req=Request(url,data=json.dumps(payload).encode(),headers={"x-goog-api-key":GEMINI_API_KEY,"Content-Type":"application/json","User-Agent":"ATLAS/75.8"},method="POST")
    t=time.perf_counter()
    try:
...
def _internal_diagnostics():
    ...
    run("Static application",lambda:(ROOT/'index.html').exists() and (ROOT/'atlas758.js').exists() and (ROOT/'atlas758.css').exists(),"Core UI assets present")
...
class H(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path=="/api/build":return self.sendj({"ok":True,"version":VERSION,"build":BUILD_NAME,"js":"atlas758.js","css":"atlas758.css","database":STORE.kind,"ai":atlas_ai_status()})
