...
VERSION="66.1"
BUILD_NAME="ATLAS 66.1 AUTO-SYNC FOOTBALL INTELLIGENCE OS"
...
def atlas_ai_ask(question):
    ...
    req=Request("https://api.openai.com/v1/responses",data=json.dumps(payload).encode(),headers={"Authorization":"Bearer "+OPENAI_API_KEY,"Content-Type":"application/json","User-Agent":"ATLAS/66.1"},method="POST")
    t=time.perf_counter()
    try:
...
def _internal_diagnostics():
    ...
    run("Static application",lambda:(ROOT/'index.html').exists() and (ROOT/'atlas661.js').exists() and (ROOT/'atlas661.css').exists(),"Core UI assets present")
...
class H(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path=="/api/build":return self.sendj({"ok":True,"version":VERSION,"build":BUILD_NAME,"js":"atlas661.js","css":"atlas661.css","database":STORE.kind,"ai":atlas_ai_status()})
