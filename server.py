...
VERSION="67.0"
BUILD_NAME="ATLAS 67.0 AUTO-SYNC FOOTBALL INTELLIGENCE OS"
...
def atlas_ai_ask(question):
    ...
    req=Request("https://api.openai.com/v1/responses",data=json.dumps(payload).encode(),headers={"Authorization":"Bearer "+OPENAI_API_KEY,"Content-Type":"application/json","User-Agent":"ATLAS/67.0"},method="POST")
    t=time.perf_counter()
    try:
...
def _internal_diagnostics():
    ...
    run("Static application",lambda:(ROOT/'index.html').exists() and (ROOT/'atlas670.js').exists() and (ROOT/'atlas670.css').exists(),"Core UI assets present")
...
class H(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path=="/api/build":return self.sendj({"ok":True,"version":VERSION,"build":BUILD_NAME,"js":"atlas670.js","css":"atlas670.css","database":STORE.kind,"ai":atlas_ai_status()})
