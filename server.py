...
VERSION="75.3"
BUILD_NAME="ATLAS 75.3 INSTANT BOOT"
...
def atlas_ai_ask(question):
    ...
    req=Request(url,data=json.dumps(payload).encode(),headers={"x-goog-api-key":GEMINI_API_KEY,"Content-Type":"application/json","User-Agent":"ATLAS/75.3"},method="POST")
    t=time.perf_counter()
    try:
...
def _internal_diagnostics():
    ...
    run("Static application",lambda:(ROOT/'index.html').exists() and (ROOT/'atlas753.js').exists() and (ROOT/'atlas753.css').exists(),"Core UI assets present")
...
class H(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path=="/api/build":return self.sendj({"ok":True,"version":VERSION,"build":BUILD_NAME,"js":"atlas753.js","css":"atlas753.css","database":STORE.kind,"ai":atlas_ai_status()})
